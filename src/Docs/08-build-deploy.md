...
Для добавления нового контента (новости, афиши, дипломы) используется специальный скрипт. Этот процесс **не требует пересборки** всего проекта.

### Команда для добавления контента

| Команда | Описание |
|---|---|
| `node scripts/add-content.js` | Обрабатывает новый контент и делает его доступным на сайте. |

### Процесс работы

1.  **Подготовка**: Новый контент (текстовый файл + изображения) помещается в подпапку внутри `scripts/staging/`.
2.  **Запуск**: Выполняется команда `node scripts/add-content.js`.
3.  **Результат**: Скрипт автоматически создает `.md` файл и переносит изображения в `public/media`, а также обновляет все необходимые манифесты.

Подробная инструкция по подготовке контента находится в файле `scripts/staging/instructions/README.md`.
