...
**Файл:** `src/pages/VideosPage.tsx`

Страница отображает сетку видеозаписей.

### Источник данных
Данные для страницы являются статическими и импортируются напрямую из массива `videos` в файле `src/data/static/videos.ts`. Динамическая загрузка не используется.

### Рендеринг видео
Каждое видео отображается в виде карточки, содержащей заголовок, описание и встроенный плеер через `<iframe>`. Это обеспечивает нативное воспроизведение с видеохостингов (Rutube, VK, YouTube).

### Вспомогательная функция `getEmbedUrl`
Внутри компонента используется функция `getEmbedUrl(video: Video)`, которая генерирует корректный URL для встраивания (`embed`) на основе поля `video.source` и `video.videoUrl`.

### Ограничения встраивания Rutube
В ходе исследования было выяснено, что API встраивания Rutube не предоставляет стандартных параметров для скрытия элементов управления или брендинга (например, кнопки "Смотреть на RUTUBE"). Поэтому плееры Rutube отображаются со стандартным интерфейсом.
